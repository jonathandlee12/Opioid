...
# 2. Specify the sheet name you want to use
sheet_name <- "Old_vs_young" 
...
genes_set1 <- c("Traf3",
                ...
                "Ldlr",
                "Adam19",
                "Fas") # Overlapping genes Morphine vs Saline Old
genes_set2 <- c("Nlrp5-ps",
                "Mdfic2",
                "Pcdhga2",
                "Gpr17",
                "Tnnc1"
...
genes_set3 <- c("Anln",
                 "Prss23") # Overlapping genes Morphine_young vs Saline_Young
...
data$highlight_group[data$gene_name %in% genes_set1] <- "Overlapping Genes Old Morphine vs Saline"
data$highlight_group[data$gene_name %in% genes_set2] <- "Top DEGs"
data$highlight_group[data$gene_name %in% genes_set3] <- "Overlapping Genes Young Morphine vs Saline"
...
custom_colors <- c(
  ...
  "Overlapping Genes Old Morphine vs Saline" = color_set1,                         # Black
  "Top DEGs" = color_set2,                              # Dark Green
  "Overlapping Genes Young Morphine vs Saline" = color_set3                                          # Magenta 
  )
...
data$highlight_group <- factor(data$highlight_group,
                               levels = c("NS",
                                          ...
                                          "Overlapping Genes Old Morphine vs Saline",
                                          "Top DEGs",
                                          "Overlapping Genes Young Morphine vs Saline"))
...
volcano_plot <- ggplot(data, aes(x = log2foldchange, y = minus_log10_padj, color = highlight_group)) +
  ...
  scale_x_continuous(breaks = c(-2, -1, -log2fc_threshold, 0, log2fc_threshold, 1, 2)) +
  theme(
